...
var seriesCounter = 0;
$.get("scan.php", function(data) {var names = (data.substring(0, data.length -1)).split(',');
...
/*		function addGenerator() {
		...
		var ChannelAChange = function() {
		  ...
		  if (aPWM.toFixed(0) == 255) {$('th#solenoidavalue').html('<strong>N/A</strong>');}
		  else {
		...
		var ChannelBChange = function() {
		  ...
		  if (bPWM.toFixed(0) == 255) {$('th#solenoidbvalue').html('<strong>N/A</strong>');}
		  else {
		  $('th#solenoidbvalue').html('<strong>' + ((bPWM - 255) * -100 / 255).toFixed(0) + '%</strong>');}
        };

		var StepperSpeed = $('#sl1').slider({min: 0.1, max: 50, step: 0.1, value: 10.0, tooltip: 'hide'})
                .on('slide', SpeedChange)
                .data('slider');
		var StepperAccel = $('#sl2').slider({min: 0, max: 100, step: 0.1, value: 20.0, tooltip: 'hide'})
                .on('slide', AccelChange)
                .data('slider');
        var StepperDecel = $('#sl3').slider({min: 0, max: 100, step: 0.1, value: 20.0, tooltip: 'hide'})
                .on('slide', DecelChange)
                .data('slider');
		var ChannelAPWM = $('#sl4').slider({min: 0, max: 255, step: 1, value: 255, tooltip: 'show', handle: 'square', orientation: 'vertical', selection: 'after', formater: function(value) {
            return ((value - 255) * -100 / 255).toFixed(0) + '% Duty Cycle';
          }})
                .on('slide', ChannelAChange)
                .data('slider');
		var ChannelBPWM = $('#sl5').slider({min: 0, max: 255, step: 1, value: 255, tooltip: 'show', handle: 'square', orientation: 'vertical', selection: 'after',
		...
		$('#stepperstart').click(function () {
            ...
            if (acceleration > 0 && deceleration > 0) {
				...
				StepperEnable.value = Pin.LOW;
				setTimeout(function() {stepper.step(numSteps, speed); 
				StartTime = new Date().getTime();
		...
		$.ajax({
			...
			type: "post",
			dataType: "html",
			data: { command: "WAVFRM?", gpibsend: "Send" },
			dataFilter: function (response) {response = response.split(';'); response.splice(0,8); response.splice(1,1); return response;},
...
function AJAXthePoints(StartPoint,FinishPoint) {
		...
		if((FinishWave-StartWave) == 0) {return;}
		...
		$.ajax({
			...
			processData: false,
			data: AJAXCommands,
			dataFilter: function(response) {response = response.slice(0, response.lastIndexOf("HTTP/1.1" )); return response;},
			success: function(data) {
		...
		$.ajax({
			...
			dataFilter: function (response) {response = response.slice(0, response.lastIndexOf("HTTP/1.1" ));response = response.split(';'); response.splice(0,5); response.splice(1,1); response.splice(3,1); return response;},
			success: function(data) {
				CurrentWaveform = data;
		...
		rotateAdjust = 1;
		StepperEnable.value = Pin.LOW;
		setTimeout(function() {stepper.step(1760, speed, acceleration, deceleration);
		StartTime = new Date().getTime();},20);
		});
		$('#rotate180').click(function () {
		$('#stepperstart').button('loading');
		rotateAdjust = 1;
		setTimeout(function() {stepper.step(3360, speed, acceleration, deceleration);
		StartTime = new Date().getTime();},20);
		});
		...
		$('#stepperstart').button('loading');
		rotateAdjust = 3;
		setTimeout(function() {stepper.step(176, 4.0);
		StartTime = new Date().getTime();},20);
		});
		...
		$('#analyzeid').click(function () {
			...
			if(AnalyzeCount == 0) {importChartData();}
			else {AnalyzeChart = $('#ushighchart').highcharts();
			AnalyzeChart.redraw();};
			AnalyzeCount++;
		});
...
//		setInterval($('#screenshotiframe').attr('src', $('#screenshotiframe').attr('src')), 2000);
		...
		if(rotateAdjust == 1) {
			rotateAdjust = 0;
			setTimeout(function() {stepper.step(-160, 1.8);}, 100);
		}
    ...
    enabled : false
		...
		series : [{
			...
			data : (function() {
				...
				var data = [], time = ((new Date()).getTime() - 1000*60*60*4), i;
